...
function findCurrents(page) {
  const status = `status[]=WAITING&status[]=ORDERED`;
  // /api/provisions?${ supplierList }&${ dateLimits }&pagination=true&page=${ page }&itemsPerPage=${ items }&order[provisionDate]=asc
//   return axios
//     .get(`/api/provisions?${status}&${dateLimits}${supplierList}`)
//     .then((response) => response.data["hydra:member"]);
  return axios
    .get(`${API_PROVISION}?${status}&pagination=true&page=${page}&itemsPerPage=25&order[orderDate]=desc`)
    .then((response) => { 
...
function findSuppliersBetween(dates, suppliers, sellers, user) {
  ...
  const dateLimits = `provisionDate[after]=${getStringDate(
    UTCDates.start
  )}&provisionDate[before]=${getStringDate(UTCDates.end)}`;
  return axios
    .get(`/api/provisions?${supplierList}&${sellerList}&${dateLimits}`)
    .then((response) => {
      return response.data["hydra:member"].sort((a, b) =>
        new Date(a.deliveryDate) < new Date(b.deliveryDate) ? -1 : 1
      );
    });
}
...
function findNeedsPerSuppliersBetween(dates, suppliers = null) {
  ...
  const supplierList = isDefined(suppliers)
...
function find(id) {
  return axios.get("/api/provisions/" + id).then((response) => response.data);
}
