...
const Accordion = ({ provisions, setProvisions }) => {
  const getSign = (status) => {
    return status === "WAITING" ? (
      <FaFileUpload size={20} className="me-2 text-danger" />
    ) : status === "ORDERED" ? (
      <FaHourglassHalf size={20} className="me-2 text-warning" />
    ) : (
  ...
  return !isDefinedAndNotVoid(provisions) ? (
    ...
    <>
      <div className="accordion pt-2 pb-5" id="accordionExample">
